...
def post_summary(pr_number, results, repo=None):
    ...
    `results` is a list of dicts with keys:
      branch, status, url, ai_impact
    (ai_impact is an advisory dict or None)
...
def main():
    ...
    for branch in branches:
        ...
        if status == "success":
            ...
            # Conflicts are never auto-resolved: the bot flags the branch for a
            # human engineer to cherry-pick and resolve manually.
            results.append(
                {
                    "branch": branch,
                    "status": "conflict",
